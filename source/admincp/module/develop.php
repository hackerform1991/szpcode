...
<body>
<div class="container">
<script type="text/javascript">parent.document.title = \'管理中心 - 安装应用\';if(parent.$(\'admincpnav\')) parent.$(\'admincpnav\').innerHTML=\'安装应用\';</script>
...
function install_ation($type)
{
	if ($type) {
		...
		$buthor = convert_charset(trim($xml->item['author']), 1);
		$bddress = trim($xml->item['address']);
		if ($one = $GLOBALS['db']->getone("select in_id from " . tname('plugin') . " where in_dir='{$dir}'")) {
			$GLOBALS['db']->query("update " . tname('plugin') . " set in_name='{$name}',in_file='{$file}',in_type={$type},in_author='{$buthor}',in_address='{$bddress}',in_addtime='" . date('Y-m-d H:i:s') . "' where in_id=" . $one);
		} else {
			$GLOBALS['db']->query("Insert " . tname('plugin') . " (in_name,in_dir,in_file,in_type,in_author,in_address,in_addtime) values ('{$name}','{$dir}','{$file}',{$type},'{$buthor}','{$bddress}','" . date('Y-m-d H:i:s') . "')");
		}
