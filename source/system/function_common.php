...
function getavatar($uid)
{
	if (is_file(IN_ROOT . './data/attachment/avatar/' . $uid . '.jpg')) {
		$bvatar = 'http://' . $_SERVER['HTTP_HOST'] . IN_PATH . 'data/attachment/avatar/' . $uid . '.jpg?v=' . time();
	} else {
		$bvatar = 'http://' . $_SERVER['HTTP_HOST'] . IN_PATH . 'static/app/avatar.jpg';
	}
	return is_ssl() ? str_replace('http://', 'https://', $bvatar) : $bvatar;
}
...
function unescape($str)
{
	...
	foreach ($br as $k => $v) {
		if (substr($v, 0, 2) == '%u') {
			$br[$k] = iconv($code, strtoupper(IN_CHARSET) . '//IGNORE', pack('H4', substr($v, -4)));
		} elseif (substr($v, 0, 3) == '&#x') {
			$br[$k] = iconv($code, strtoupper(IN_CHARSET) . '//IGNORE', pack('H4', substr($v, 3, -1)));
		} elseif (substr($v, 0, 2) == '&#') {
			$br[$k] = iconv($code, strtoupper(IN_CHARSET) . '//IGNORE', pack('H4', substr($v, 2, -1)));
		}
	}
	return SafeSql(join('', $br));
}
...
{
	$brray = isset($_POST[$key]) ? $_POST[$key] : NULL;
	if (empty($brray)) {
		$value = 0;
	} else {
		for ($i = 0; $i < count($brray); $i++) {
			$brr[] = intval($brray[$i]);
		}
		$value = implode(',', $brr);
	}
...
function dstrpos($string, $brr)
{
	if (!empty($string)) {
		foreach ((array) $brr as $v) {
